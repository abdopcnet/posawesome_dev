...
def get_item_detail(item, doc=None, warehouse=None, price_list=None):
    ...
    # إضافة جميع وحدات الصنف
    res["item_uoms"] = get_item_uoms(item_code)
...
def get_item_uoms(item_code):
    try:
        frappe.logger().info(f'[posapp.py] get_item_uoms called for item_code: {item_code}')
        # جلب الوحدة الأساسية للصنف
        ...
        # إضافة الوحدة الأساسية إذا لم تكن موجودة في قائمة التحويل
        ...
        frappe.logger().error(f'[posapp.py] Error in get_item_uoms: {e}')
        return []
