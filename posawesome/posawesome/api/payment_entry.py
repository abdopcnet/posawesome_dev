...
def process_pos_payment(payload):
    try:
        ...
        data = __dict(data)
        if not data.pos_profile.get("posa_use_posa_pos_awesome_payments"):
            frappe.logger().error('[payment_entry.py] POS Awesome Payments is not enabled for this POS Profile')
        ...
        if (
            ...
            for mpesa_payment in data.selected_mpesa_payments:
                try:
                    frappe.logger().info(f'[payment_entry.py] Submitting MPESA payment: {mpesa_payment.get("name")}, customer: {customer}')
                    ...
                    frappe.logger().error(f'[payment_entry.py] Error submitting MPESA payment: {e}')
                    errors.append(e)
