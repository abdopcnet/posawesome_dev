      ...
      >
        <v-col cols="8" class="pb-0 mb-0 pr-0 pt-0">
          <v-autocomplete
            ...
            :no-data-text="__('No shipping costs available')"
            hide-details
              ...
              <td :colspan="columns.length" class="ma-0 pa-0">
                ...
                <v-row class="ma-0 pa-0">
                  ...
                  <v-col align="center" cols="4" v-if="item.posa_offer_applied">
                    <v-checkbox
                      dense
                      :label="__('Promotional Scheme Applied')"
                  ...
                  >
                    <v-text-field
                      ...
                      background-color="white"
                      hide-details
                      :value="formatFloat(item.actual_batch_qty)"
                      disabled
                    ></v-text-field>
                  </v-col>
                  <v-col
          ...
          <v-row no-gutters class="pa-1 pt-9 pr-1">
            ...
            >
              <v-text-field
                ...
                "
                :rules="[isNumber]"
                :label="__('Apply Additional Discount')"
                suffix="%"
...
export default {
  ...
  data() {
    return {
      ...
      items_headers: [
        { title: __("Item Name"), align: "start", sortable: true, key: "item_name" },
  ...
  computed: {
    ...
    dynamicHeaders() {
      return this.items_headers;
    },
    total_qty() {
      return this.items.reduce((sum, item) => sum + flt(item.qty), 0);
    },
    Total() {
      return this.items.reduce((sum, item) => sum + flt(item.qty) * flt(item.rate), 0);
    },
    subtotal() {
      return this.Total - flt(this.discount_amount) + flt(this.delivery_charges_rate);
    },
  ...
  methods: {
        ...
        if (item.has_serial_no && item.to_set_serial_no) {
          if (cur_item.serial_no_selected.includes(item.to_set_serial_no)) {
            evntBus.emit("show_mesage", {
              text: __(`This serial number {0} is already added!`, [
    ...
    get_new_item(item) {
      ...
      // Maintain the list of units for the item
      new_item.item_uoms = item.item_uoms || [];
      
      // Add the base unit if it's not in the units list
      ...
      // Data correction: Ensure each unit has the required fields
      new_item.item_uoms = new_item.item_uoms.map(uom => {
        // If the unit is a string, convert it to an object
        if (typeof uom === 'string') {
          return { uom: uom, conversion_factor: 1 };
        } 
        // If it's an object, ensure the required fields exist
        ...
        // If null or undefined, use the base unit
        else {
          return { uom: item.stock_uom || 'Nos', conversion_factor: 1 };
        }
      }).filter(uom => uom && uom.uom); // Filter out any invalid units
      
      // Set the correct conversion factor based on the selected unit
    ...
    get_invoice_doc() {
      ...
      if (this.invoice_doc.name) {
        ...
        console.info('[Invoice] Cloning existing invoice_doc:', doc);
      }
      ...
      doc.branch = this.pos_profile.branch || "";
      console.info('[Invoice] Built invoice doc:', doc);
      return doc;
    },
      ...
      this.items.forEach((item) => {
        if (
          ...
          if (item.discount_amount && this.flt(item.discount_amount) > 0) {
            ...
            if (
              ...
              console.error('[Invoice] Discount percentage too high for item:', item);
              evntBus.emit("show_mesage", {
                text: __(
        ...
        if (this.stock_settings.allow_negative_stock != 1) {
          if (
            ...
            console.error('[Invoice] Insufficient stock for item:', item);
            evntBus.emit("show_mesage", {
              text: __(
        ...
        if (item.qty == 0) {
          console.error('[Invoice] Item quantity is zero:', item);
          evntBus.emit("show_mesage", {
        ...
        if (
          ...
          console.error('[Invoice] Discount percentage exceeds max for item:', item);
          evntBus.emit("show_mesage", {
            text: __(`Allowed discount for item {0} {1}`, [
              item.item_name,
              item.max_discount,
            ]),
            color: "error",
        ...
        if (item.has_serial_no) {
          if (
            ...
            console.error('[Invoice] Invalid serial numbers for item:', item);
            evntBus.emit("show_mesage", {
              text: __(`Serial numbers selected for item {0} are invalid`, [
                item.item_name,
              ]),
              color: "error",
            });
        ...
        if (item.has_batch_no) {
          if (item.stock_qty > item.actual_batch_qty) {
            console.error('[Invoice] Not enough batch quantity for item:', item);
            evntBus.emit("show_mesage", {
              text: __(
        ...
        if (this.pos_profile.posa_allow_user_to_edit_additional_discount) {
          ...
          if (clac_percentage > this.pos_profile.posa_max_discount_allowed) {
            console.error('[Invoice] Additional discount percentage too high:', clac_percentage);
            evntBus.emit("show_mesage", {
              text: __(`Discount percentage cannot be higher than {0}%`, [
                this.pos_profile.posa_max_discount_allowed,
              ]),
              color: "error",
            });
        ...
        if (this.invoice_doc.is_return) {
          if (this.subtotal >= 0) {
            console.error('[Invoice] Return invoice subtotal is invalid:', this.subtotal);
            evntBus.emit("show_mesage", {
              text: __(`Return invoice total is invalid`),
              color: "error",
            });
            value = false;
            return value;
          ...
          if (Math.abs(this.subtotal) > Math.abs(this.return_doc.total)) {
            console.error('[Invoice] Return invoice subtotal exceeds original:', this.subtotal, this.return_doc.total);
            evntBus.emit("show_mesage", {
          ...
          this.items.forEach((item) => {
            ...
            if (!return_item) {
              console.error('[Invoice] Return item not found in original invoice:', item);
              evntBus.emit("show_mesage", {
                text: __(
              ...
              console.error('[Invoice] Return item quantity invalid:', item, return_item);
              evntBus.emit("show_mesage", {
                text: __(`Quantity for item {0} cannot be greater than {1}`,[
                  item.item_name,
                  return_item.qty,
                ]),
                color: "error",
    ...
    calc_uom(item, value) {
      ...
      // Search for the conversion factor for the selected unit
      ...
      // Save the base price if it's not already saved
      ...
      // Recalculate the price based on the conversion factor
      if (item.batch_price) {
    ...
    applyNewOffer(offer) {
      ...
      if (offer.offer === "Loyalty Point") {
        evntBus.emit("show_mesage", {
          text: __("Loyalty points offer applied"),
          color: "success",
        });
      }
    ...
    print_draft_invoice() {
      if (!this.pos_profile.posa_allow_print_draft_invoices) {
        evntBus.emit("show_mesage", {
          text: __(`You are not allowed to print pending invoices`),
          color: "error",
        });
        return;
      }
...
.v-btn.light-blue.lighten-1 {
  background-color: #4FC3F7 !important;
}

.v-btn.grey.lighten-3 {
