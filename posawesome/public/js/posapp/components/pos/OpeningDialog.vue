  ...
  setup(props) {
    ...
    const get_opening_dialog_data = () => {
      frappe.call({
        ...
        callback: function (r) {
          if (r.message) {
            console.info('[OpeningDialog] Loaded opening dialog data:', r.message);
            ...
            console.error('[OpeningDialog] Failed to load opening dialog data:', r);
          }
        },
      });
    };
    ...
    const submit_dialog = () => {
      if (!payments_methods.value.length || !company.value || !pos_profile.value) {
        console.error('[OpeningDialog] Missing required fields for submit:', {
          payments_methods: payments_methods.value,
      ...
      console.info('[OpeningDialog] Submitting opening voucher:', {
        pos_profile: pos_profile.value,
        company: company.value,
      ...
      frappe
        ...
        .then((r) => {
          if (r.message) {
            console.info('[OpeningDialog] Opening voucher created:', r.message);
            evntBus.emit('register_pos_data', r.message);
            evntBus.emit('set_company', r.message.company);
            close_opening_dialog();
            is_loading.value = false;
          } else {
            console.error('[OpeningDialog] Failed to create opening voucher:', r);
          }
        });
    };
