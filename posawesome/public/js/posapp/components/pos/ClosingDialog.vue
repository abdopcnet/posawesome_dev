<template>
  <v-row justify="center">
    <v-dialog v-model="closingDialog" max-width="900px">
      <v-card>
        <v-card-title>
          <span class="headline primary--text">{{ __('Close Cashier Shift') }}</span>
        </v-card-title>
                ...
                >
                  ...
                  <template v-slot:[`item.expected_amount`]="{ item }">
                    ...
                    <span v-if="item && typeof item.expected_amount !== 'undefined'">
                      <span v-if="(() => { try { console.info('[ClosingDialog] expected_amount:', item.expected_amount); return true; } catch (e) { console.error('[ClosingDialog] Error logging expected_amount:', e); return false; } })()">
...
export default {
  ...
  setup() {
    ...
    const headers = ref([
      ...
      {
        title: __('Expected Total'),
        align: 'end',
        sortable: true,
        key: 'opening_amount',
      },
      {
        title: __('Closing Total'),
        key: 'closing_amount',
        align: 'end',
        sortable: true,
      },
